...
def parse_fields(str, types):
    ...
    for i in fields_.split(","):
        ...
        if mo:
            t["name"] = mo.group(1)
            t["is_array"] = 2
            t["array_size1_d"], t["array_size1_u"], t[
                "array_size1_o"] = f(mo.group(2))
            t["array_size2_d"], t["array_size2_u"], t[
            ...
            if mo:
                t["name"] = mo.group(1)
                t["is_array"] = 1
                t["array_size_d"], t["array_size_u"], t[
...
class Field:
    ...
    def get_python_declar(self):
        ...
        if self.dataio_type == "uint8":
            return "B"
        elif self.dataio_type == "uint16":
            return "BBBBBBB"
        elif self.dataio_type == "uint32":
            return "I"
        elif self.dataio_type == "sint32":
            return "i"
        elif self.dataio_type == "sint16":
            return "bb"
        elif self.dataio_type == "sint8":
            return "b"
        elif self.dataio_type == "bool8":
            return "?"
        elif self.dataio_type == "string":
            return "z"
        elif self.dataio_type == "tech_list":
            return "B"
        elif self.dataio_type == "requirement":
            return "BbB??"
        elif self.dataio_type == "float10000":
            return "f"
            # FIXME: float sometimes returns nan
        elif self.dataio_type == "float1000000":
            return "f"
            # FIXME
        elif self.dataio_type == "bitvector":
            return "$"
        elif self.dataio_type == "diplstate":
            return "BBBBBBBBB"
            # FIXME: this needs to be checked...
        elif self.dataio_type == "bit_string":
            return "z"
            # FIXME
        else:
            return self.dataio_type
...
''' % self.get_dict(vars())
    ...
    def get_put(self):
        ...
        arr_types = ["string", "bit_string", "city_map",
...
class Variant:
    ...
    def __init__(self, poscaps, negcaps, name, fields, packet, no):
        ...
        self.keys_arg = ", ".join(
            ["real_packet->" + x.name for x in self.key_fields])
        if self.keys_arg:
            self.keys_arg = ",\n    " + self.keys_arg
        ...
        if self.want_force:
            self.extra_send_args = self.extra_send_args + \
                ', bool force_to_send'
...
'''
    ...
    def get_delta_receive_body(self):
        key1 = ["    %s %s = real_packet->%s;" %
                (x.struct_type, x.name, x.name) for x in self.key_fields]
        key2 = ["    real_packet->%s = %s;" %
                (x.name, x.name) for x in self.key_fields]
        key1 = "\n".join(key1)
        key2 = "\n".join(key2)
        if key1:
            key1 = key1 + "\n\n"
...
class Packet:
    ...
    def __init__(self, str, types):
        ...
        if not self.no_packet:
            self.extra_send_args = ', const struct %(name)s *packet' % self.__dict__ + \
                self.extra_send_args
            self.extra_send_args2 = ', packet' + self.extra_send_args2
    ...
    def parse_fields(self):
        self.body_fields = []
        for field in self.key_fields + self.other_fields:
            self.body_fields.append(
                {'name': field.name, 'type': field.get_python_declar(), 'field': field})
    # '''
...
''' % self.get_dict(vars())
        for v in self.variants:
            ...
            no = v.no
            result = result + \
                '    case %(no)s: return receive_%(name2)s(pc, type);\n' % self.get_dict(
                    vars())
        result = result + \
...
''' % self.get_dict(vars())
        ...
        for v in self.variants:
            ...
            no = v.no
            result = result + \
                '    case %(no)s: return send_%(name2)s(%(args)s);\n' % self.get_dict(
                    vars())
        result = result + \
